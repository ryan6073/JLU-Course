    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(listen,&QTcpServer::newConnection,[=](){
        ...
        connect(conn,&QTcpSocket::readyRead,[=](){
            ...
            if (!QJsonDocument::fromJson(readMessage).isObject())
            {
                ...
                if(qqConnectMap.contains(conn))
                {
                    distributeMessageQQ(readMessage);
                }
                else
                {
                    distributeMessageWeChat(readMessage);
                }
    ...
    DataHandler::GetInstance()->readFunction();
    DataHandler::GetInstance()->readWeChat();
}
...
void MainWindow::distributeMessage(const QByteArray &data)
{
    ...
    for (int i = 0;i < connList.size();i++)
    {
        ...
        {
            if(qqConnectMap.contains(connList[i]))
            {
                qqConnectMap.remove(connList[i]);
            }
            else if(weChatConnectMap.contains(connList[i]))
            {
                weChatConnectMap.remove(connList[i]);
            }
            //如果有链接是无效的就移除
            delete connList[i];
            connList.removeAt(i);
...
void MainWindow::distributeMessageQQ(const QByteArray &data)
{
    if(data.isEmpty())
    {
        return;
...
void MainWindow::distributeMessageWeChat(const QByteArray &data)
{
    if(data.isEmpty())
    {
        return;
    ...
    for (int i = 0;i < connList.size();i++)
    {
        if(connList[i]->state() == QTcpSocket::UnconnectedState)
        {
            if(weChatConnectMap.contains(connList[i]))
            {
                weChatConnectMap.remove(connList[i]);
            }
            //如果有链接是无效的就移除
...
void MainWindow::analysisJson(QTcpSocket *conn, QVariantMap& map)
{
    if(map["function"] == "qq")
    {
        ...
        if(map["operation"] == "register")
        {
            ...
            else if(DataHandler::GetInstance()->registerQQ(account,"user",date,password,identification))
            {
        ...
        else if(map["operation"] == "login")
        {
            ...
            {
                qqConnectMap[conn] = account;
                map["identification"] = DataHandler::GetInstance()->getIdentification(account);
                map["result"] = "1";
            }
            else
            {
        ...
        {
            bool ret = false;
            QQInfo_Zhujs selfInfo = DataHandler::GetInstance()->queryQQ(account,ret);
        ...
        else if(map["operation"] == "chat")
        {
            ...
            for(auto iter=qqConnectMap.begin();iter!=qqConnectMap.end();++iter)
            {
        ...
        else if(map["operation"] == "autologin")
        {
            map["identification"] = DataHandler::GetInstance()->getIdentification(account);
            conn->write(QJsonDocument::fromVariant(map).toJson());
        }
    }
    else if(map["function"] == "function")
    {
        if (map["operation"] == "open")
        {
            ...
            DataHandler::GetInstance()->functionMap[identification].qqOpen=qq;
            DataHandler::GetInstance()->functionMap[identification].weChatOpen=wechat;
            DataHandler::GetInstance()->functionMap[identification].weiBoOpen=weibo;
            map["result"]="1";
            DataHandler::GetInstance()->writeFunction();
            conn->write(QJsonDocument::fromVariant(map).toJson());
        }
    }
    else if(map["function"] == "wechat")
    {
        QString account = map["account"].toString();
        if(map["operation"] == "register")
        {
            ...
            else if(DataHandler::GetInstance()->registerWeChat(account,"user",date,password,identification))
            {
                DataHandler::GetInstance()->writeWeChat();
                map["result"] = "1";
            }
            else
            {
        ...
        {
            QString password = map["password"].toString();
            if(DataHandler::GetInstance()->loginWeChat(account,password))
            {
                weChatConnectMap[conn] = account;
                map["identification"] = DataHandler::GetInstance()->getIdentification(account,false);
                map["result"] = "1";
            }
            else
            {
