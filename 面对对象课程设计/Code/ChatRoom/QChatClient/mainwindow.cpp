    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->treeWidget->header()->setFont(QFont("楷体"));

    connect(&loginView, SIGNAL(loginAccount(QString, QString)), this, SLOT(loginAccount(QString, QString)));
    connect(&loginView, SIGNAL(registerAccount(QString, QString)), this, SLOT(registerAccount(QString, QString)));
...
MainWindow::~MainWindow()
{
    if (client)
    {
        QByteArray getConnectMessage = QTime::currentTime().toString("hh:mm:ss").toUtf8() + " [聊天室]" + clientName.toUtf8() + " 断开连接!";
...
{
    QString qss;
    QFile file(":/Resource/lightblue.css");
    ...
    if (map["function"] == "qq")
    {
        if (map["operation"] == "login")
        {
            if (map["account"] == tryAccount && map["password"] == tryPassword)
            {
                if (map["result"] == "1")
                {
                    QMessageBox::information(this, "系统消息", QString("[%1]登录成功").arg(tryAccount));
                ...
                {
                    QMessageBox::warning(this, "系统消息", QString("[%1]登录失败").arg(tryAccount));
                    currentUser = "";
                }
                loginView.hide();
            }
        ...
        else if (map["operation"] == "register")
        {
            if (map["account"] == tryAccount && map["password"] == tryPassword)
            {
                if (map["result"] == "1")
                {
                    QMessageBox::information(this, "系统消息", QString("[%1]注册成功").arg(tryAccount));
                    currentUser = tryAccount;
                }
                else
                {
                    QMessageBox::warning(this, "系统消息", QString("[%1]注册失败").arg(tryAccount));
                    currentUser = "";
                }
                loginView.hide();
            }
        ...
        else if (map["operation"] == "append" || map["operation"] == "delete" || map["operation"] == "alter" || map["operation"] == "get")
        {
            ...
            if (map["account"] == currentUser && map["result"] == "1")
            {
                ...
                QMessageBox::information(this, "系统消息", operation + "成功");
            }
            else
            {
                QMessageBox::information(this, "系统消息", operation + "失败");
            }
        }
        ...
        else if (map["operation"] == "update")
        {
            if (map["account"] == currentUser && map["result"] == "1")
            {
                QMessageBox::information(this, "系统消息", "更新个人信息成功");
                infoDialog.close();
            ...
            {
                QMessageBox::information(this, "系统消息", "更新个人信息失败");
            }
        }
...
void MainWindow::on_treeWidget_customContextMenuRequested(const QPoint& pos)
{
    ...
    if (ret == addFriend)
    {
        QString number = QInputDialog::getText(this, "号码查找", "请输入需要查找的QQ号");
        if (!number.isEmpty())
        {
            QVariantMap map;
            map["function"] = "qq";
    ...
    if(!client || client->state() != QAbstractSocket::ConnectedState)
    {
        ...
        connect(client, &QTcpSocket::stateChanged, [=]() {
            if (client->state() == QAbstractSocket::ConnectedState)
            {
                ...
                ui->clientState->setText("在线状态：待登录");
                //告诉服务器链接成功了
...
void MainWindow::loginAccount(QString account, QString password)
{
    tryAccount = account;
...
void MainWindow::registerAccount(QString account, QString password)
{
    tryAccount = account;
    tryPassword = password;
